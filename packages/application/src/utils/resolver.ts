...
export function resolveModuleOrder(
...
): ApplicationModuleMeta<any, any>[] {
  const result: ApplicationModuleMeta[] = [];
  ...
  const visit = (mod: ApplicationModuleMeta, stack: string[] = []) => {
    // If already visited, skip
    if (visited.has(mod.name)) return;

    // If in the current stack, we have a cycle
    if (stack.includes(mod.name)) {
      throw new Error(`Cycle detected: ${[...stack, mod.name].join(" → ")}`);
    }
