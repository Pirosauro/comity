...
export class Lifecycle {
   ...
   */
  seal(): Result<KernelState, KernelInvalidStateError> {
    // Can only seal from "open" state
    if (this.#state !== "open") {
      return failure(
        new KernelInvalidStateError({
          action: "seal",
          state: this.#state,
        }),
      );
   ...
   */
  start(): Result<KernelState, KernelInvalidStateError> {
   ...
   */
  stop(): Result<KernelState, KernelInvalidStateError> {
