...
type Options<C, P> = {
  debug: boolean;
  components: Record<string, () => Promise<{ default: C } | undefined>>;
  integrations: Record<
    string,
    (component: C, props: P, element: HTMLElement) => Promise<void> | void
  >;
};
 ...
 * @return {Promise<void>}
 */
export const createClient = async <C = any, P = any>({
  debug,
  components,
  integrations,
  ...
  logMessage(
    "Client initialized with " + Object.keys(components).length + " components",
    "info"
  );
  ...
  if ("customElements" in window) {
    window.customElements.define("comity-island", IslandElement);
  }
};
