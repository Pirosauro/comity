...
export enum ServiceFlags {
  /** Transient service: new instance created on every access */
  TRANSIENT = 0,
 ...
 * container.register('requestId', () => crypto.randomUUID(), ServiceFlags.TRANSIENT);
 *
 * // Use services
...
export class Container {
   ...
   * - `ServiceFlags.TRANSIENT`: New instance created on every access, no caching
   *
  ...
  public get<T>(key: string): T {
    const flags = this.#flags.get(key) ?? ServiceFlags.TRANSIENT;
    ...
    if (!factory) {
      throw new NotFoundError("Service not registered", {
        service: key,
        available: Array.from(this.#factories.keys()),
      });
    }
