...
export const preactRendererMiddleware = (
  ...
  options: PreactStreamRendererOptions
): MiddlewareHandler => {
  return async (ctx, next) => {
    async function renderer(children: VNode, props?: PropsWithChildren<{}>) {
      ...
      return stream(ctx, async (stream) => {
        ...
        try {
          const rendered = await options?.renderer(node as VNode, options);
